...
from PyQt6.QtGui import QPixmap, QPainter, QLinearGradient, QColor, QBrush, QFont

JSON = "json/checklist.json"

def fetch_checklist():
    try:
        filepath = os.path.join(os.path.dirname(__file__), JSON)
...
class ColorPreservingDelegate(QStyledItemDelegate):
    def paint(self, painter, option, index):
        ...
        # Background personalizzato (anche trasparente)
        if bg:
...
class OWASPChecklistApp(QWidget):
    ...
    def loadOfflineReferenceData(self):
        try:
            filepath = os.path.join(os.path.dirname(__file__), "json/wstg_offline_data.json")
            with open(filepath, "r", encoding="utf-8") as f:
                return json.load(f)
        except Exception as e:
    ...
    def loadProgressFromFile(self):
        filepath = os.path.join(os.path.dirname(__file__), "json/progress.json")
    ...
    def loadCategoryDescriptions(self):
        try:
            filepath = os.path.join(os.path.dirname(__file__), "json/category_descriptions.json")
            if os.path.exists(filepath):
                with open(filepath, "r", encoding="utf-8") as f:
                    return json.load(f)
        except Exception as e:
    ...
    def loadOwaspTop10(self):
        try:
            path = os.path.join(os.path.dirname(__file__), "json/owasp_top_10.json")
    ...
    def showMappingTable(self):
        ...
        dialog.setGeometry(200, 200, 1300, 500)
        ...
        html = ("<table border='1' cellpadding='6' cellspacing='0' "
                "style='border-collapse: collapse; color:#ffffff; table-layout: fixed; width: 100%;'>"
                "<col style='width: 50%;'><col style='width: 50%;'>")

        html += "<tr style='background-color:#404247;'><th>Categoria WSTG</th><th>OWASP Top 10 (2021)</th></tr>"
        for wstg_cat, owasp_refs in mapping_data.items():
            html += f"<tr><td>{wstg_cat}</td><td>{owasp_refs}</td></tr>"
        html += "</table>"

        html_box = QTextEdit()
        html_box.setMinimumWidth(480)
        html_box.setHtml(html)
        html_box.setReadOnly(True)
        html_box.setStyleSheet("background-color: #2a2b2e; color: #ffffff; font-size: 13px;")
    ...
    def saveStatus(self):
        filepath = os.path.join(os.path.dirname(__file__), "saves/progress_temp.json")
...
#   Pagina iniziale     #
#########################
from PyQt6.QtWidgets import QSplashScreen, QLabel, QProgressBar, QGraphicsOpacityEffect, QApplication
from PyQt6.QtCore import Qt, QTimer, QPropertyAnimation, QEasingCurve, QEventLoop, QPoint
...
class ModernSplashScreenPNG(QSplashScreen):
    ...
    def start(self):
        ...
        self.status_timer.start(700)  # un messaggio ogni 0.8 secondi
    ...
    def simulateProgress(self):
        for i in range(1, 101):
            QTimer.singleShot(i * 42, lambda v=i: self.progress.setValue(v))
...
if __name__ == "__main__":
    ...
    splash = ModernSplashScreenPNG("asset/logo.png")
    splash.start()
